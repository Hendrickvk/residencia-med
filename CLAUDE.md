...
- The admin-only screens (Banco de Questões, Nova Questão, Importar Planilha, Sincronizar MediaFire, plus a Materiais screen where the admin adds and deletes materials) live permanently in the original Streamlit app (`app.py`, `ui.py`, `auth.py`) — decided explicitly, not migrating them (see `MIGRACAO.md` §4/§5).
