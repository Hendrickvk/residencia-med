...
- The official-edition simulado (`POST /simulados/oficial`, "Prova oficial" tab) depends on `questoes.edicao` (e.g. `"2025/1"`) and `questoes.numero_prova` (question number in the INEP booklet). Both were backfilled for every Revalida edition and for ENAMED 2025 (booklet 1 numbering) by matching the official PDFs; any new import of an official exam must fill them, or the edition won't be listed or will come out of order.
