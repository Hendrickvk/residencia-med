...
### Reviewing the explanations (in progress — 302 of 1074 done)
The explanations were all written here, so nobody independent has ever read them. The review started on 2026-09-16; per-question status and notes live in `backups/revisao_explicacoes.json` (gitignored) and show up at the top of the Streamlit **Revisar explicações** screen. **Done: USP 2026 (113), ENAMED 2025 (90), Revalida 2026/1 (99). Next: Revalida 2025/2, UNICAMP 2023, then the older editions.** Tools: `scripts/dump_explicacoes.py <banca> <edicao> [--de N --ate N]` to read them, `scripts/corrigir_explicacoes.py <json>` to rewrite (dry run, backup, `--aplicar`), `scripts/substituir_imagem.py` for figures.
...
HTTPS via the free DuckDNS subdomain (`conduta.duckdns.org`, `admin.conduta.duckdns.org`) is already committed (`deploy/Caddyfile`, env files) but **not applied**: the SSH key authorized on the server is on another computer. This machine has its own key (`~/.ssh/conduta-oracle`, alias `residencia-med`) and already trusts the server's host key (checked against the recorded fingerprint on 2026-09-15), but `ssh residencia-med` still ends in `Permission denied (publickey)` — the unblock is to append this machine's `conduta-oracle.pub` to the server's `authorized_keys` from the other computer, or to bring the instance's original key here; nothing about it can be done from this machine. Every commit since then (Triagem redesign, official-exam simulado and `questoes_provas`, especialidades taxonomy, the Neon dead-connection fix, spaced-repetition phases 1–3, the dead-code cleanup, the removal of study materials, the temas and `tipo_pergunta` classification, the 7-item performance tracking) is waiting on that deploy — steps and the server host key fingerprint are in `HISTORICO.md`. The database is shared, so data changes are already live while the server runs old code. `DEPLOY.md` still describes the IP setup; update it only after the HTTPS deploy is tested, and don't close port 8080 before that. Use the `residencia-med` SSH alias and keep remote commands short — long composite SSH commands have tripped the safety classifier.
